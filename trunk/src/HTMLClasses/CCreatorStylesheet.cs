...
namespace GEDmill.HTMLClasses
{
    ...
    public class CCreatorStylesheet : CCreator
    {
        // Name of the CSS file we are creating
        private string m_sCssFilename;

        // Name of the background image, to use in the CSS background-image attribute.
        private string m_sBackgroundImageFilename;

        public CCreatorStylesheet( CGedcom gedcom, IProgressCallback progress, string sW3cfile, string sCssFilename, string sBackgroundImageFilename ) : base( gedcom, progress, sW3cfile )
        {
        ...
        public void Create()
        {
          LogFile.TheLogFile.WriteLine( LogFile.DT_HTML, LogFile.EDebugLevel.Note, "CCreatorStylesheet::Create()" );
          ...
          try
          {
            if( File.Exists( m_sCssFilename ) )
            {
              // Delete any current file
              File.SetAttributes( m_sCssFilename, FileAttributes.Normal );
              File.Delete( m_sCssFilename );
            }
            fs = new FileStream( String.Concat( MainForm.s_config.m_sOutputFolder, "\\", MainForm.s_config.m_sStylesheetFilename, ".css" ), FileMode.Create );
            sw = new StreamWriter( fs );

            sw.WriteLine(String.Concat( "/* CSS file generated by ", MainForm.m_sSoftwareName, " on ", date, " */") );
            sw.WriteLine("BODY {");
            sw.WriteLine("  background-color: #f1f4e9;");
            if( m_sBackgroundImageFilename != "" )
            {
              sw.WriteLine(String.Concat("  background-image: url(", m_sBackgroundImageFilename, ");"));
            }
            ...
            if( MainForm.s_config.m_bAllowMultipleImages )
            {
              sw.WriteLine("DIV#main {");
              int nMainWidth = (760-(int)MainForm.s_config.m_uMaxImageWidth);
              if( nMainWidth < 0 )
              {
                nMainWidth = 600;
              }
            ...
            sw.WriteLine("  width:" + (MainForm.s_config.m_uMaxImageWidth+2) + "px;");
            sw.WriteLine("  height:" + (MainForm.s_config.m_uMaxImageHeight+40) + "px;");
            ...
            sw.WriteLine("  width:" + (MainForm.s_config.m_uMaxThumbnailImageWidth+3) + "px;");
            sw.WriteLine("  height:" + (MainForm.s_config.m_uMaxThumbnailImageHeight+3) + "px;");
            sw.WriteLine("  text-align:center;");
            sw.WriteLine("}");
          ...
          {
            LogFile.TheLogFile.WriteLine( LogFile.DT_HTML, LogFile.EDebugLevel.Error, "Caught IO Exception(5) : " + e.ToString() );
          }
          catch( ArgumentException e )
          {
            LogFile.TheLogFile.WriteLine( LogFile.DT_HTML, LogFile.EDebugLevel.Error, "Caught Argument Exception(5) : " + e.ToString() );
          }
          catch( System.UnauthorizedAccessException e )
          {
            LogFile.TheLogFile.WriteLine( LogFile.DT_HTML, LogFile.EDebugLevel.Error, "Caught UnauthorizedAccessException(5) : " + e.ToString() );
          }
          finally
          {
            if( sw != null )
